...
export interface BinResult { binType: BinColor; passed: boolean; aiConfidence: number; contaminationDetail?: string | null; imagePath?: string | null; }
...
export interface Complaint { id: number; title: string; description: string; status: string; wardId?: number; location?: string; imagePath?: string; createdAt?: string; upvotes: number; category?: string; }
export interface WorkerScanDetails {
  ...
  // GPS Proximity (soft — informational only, never blocks pickup)
  gpsStatus?: 'WITHIN_RANGE' | 'OUT_OF_RANGE' | 'FIRST_VISIT_MATCHED' | 'FIRST_VISIT_MISMATCH' | 'FIRST_VISIT_NO_REG' | 'NO_GPS';
